...
> - **`TransferInInit::on_error`** — demoted from a fix to a cross-repo question: an error ack means the vault ran and *refused* a mechanical send-back of proceeds it holds, and every plausible cause (balance desync, malformed params) is deterministic, for which the default `Err`-freeze is correct. Genuinely transient conditions surface as timeouts, which `on_timeout` already retries. Blindly re-entering with `Ok` would loop a full round-trip per iteration against a vault that refuses every time. Add `on_error` only if `ibc-solray` can return a *transient* error ack for `TransferOut` — to be answered there.
