...
| | |
|---|---|
| **Status** | **Proposed (gated), partially implemented** — the controller legs' safe-delivery removal landed 2026-08-04 (#763, §2 amendment); the `TransferOut` leg remains blocked on an external `ibc-solray` capability decision (the two pivotal questions in §8). |
| **Date** | 2026-07-27 |
...
*(Description as of the ADR date, pre-#763 — since then the segment survives only on the `TransferOut` leg; see the §2 amendment.)* Every DEX response delivered to a `lease` instance is processed through a four-box "safe-delivery" segment — `ResponseDelivery` + a `reply_on_error` self-submessage + a `TimeAlarms` retry loop. It is documented box-by-box in `protocol/docs/remote-lease-callback-flow.md` (boxes A–D, lines 44–110) and implemented in `protocol/packages/dex/src/impl_/resp_delivery/mod.rs`.
...
> **Amended 2026-08-04 (#763).** The partial removal argued against above is now an accepted trade-off, and it has landed: the `SwapExactInRespDelivery` and `TransferInInitRespDelivery` variants are gone, their `on_response` arms rewired to synchronous `Handler::on_response` — Phase C, early, for the two controller legs. What was over-weighted above: the retained machinery serves exactly one variant (`TransferOutRespDelivery`), and "two callback models" is not fragmentation but the honest picture — the two transports genuinely have different retry semantics (controller: relayer-retried ack; Neutron sudo: gas-capped, failure-queue-parked), and hiding that behind a uniform wrapper is what made the wrong model easy to form.
>
> The Phase-B prerequisite is **withdrawn for these two legs**, on the freeze posture: deterministic errors are bugs; the lease freezes, unchanged, until a fixed deploy. The prerequisite rested on pre-mortem item 1's "infinite relayer retry" — but the pre-removal mechanism gave the same deterministic failure an infinite *time-alarm* retry with no absorber and no terminal, so the rewire relocates an existing loop rather than creating one, and improves it: nothing commits (lease and controller stay in agreement, vs. the old ack-committed-but-lease-spinning divergence), and the retained packet commitment is itself the re-drive — a fixed deploy makes the next redelivery succeed with no buffered state and no `Heal` call. Consequently §8 Q6 is answered as (b), and the Phase-B items are re-scoped (see Phase B).

The gate is the sub-decision in §4 (Option A vs B) plus the two pivotal `ibc-solray` capability questions (§8). **This ADR is Proposed; the gate is unresolved** — it now blocks only the `TransferOut` leg (Phases A, E, and the remainder of D).

---

## 3. Scope boundary and the honest-IBC replay argument

Idempotency is **not** a new concern for state-advancing handlers under relayer-retry. A controller ack is retried **only** when its `WasmMsg::Execute(RemoteLeaseCallback)` reverted (`Err`) — i.e. precisely when state did **not** advance — so re-running the handler on unchanged persisted state is idempotent by construction. On `Ok` the packet commitment is deleted, so there is no re-delivery. The current honest-IBC dedup argument is thus *replaced by a stronger* commitment-deleted-on-`Ok` invariant. The **terminal / late-ack absorbers** must stay idempotent (they can legitimately receive a duplicate on the UNORDERED channel).

> **Amended 2026-07-31 (PR #765).** This paragraph previously also required the late-ack absorbers to *stay authenticated*, "via the already-wired `authz_remote_callback`". That requirement is withdrawn for the lease-state absorbers, and the governing rule is now:
>
> **An absorber that emits must authenticate; an absorber that emits nothing need not.**
>
> Authentication on an absorber protects no state and no funds — a late-ack absorber mutates nothing and moves nothing, so an unauthenticated caller achieves a self-funded no-op. What it does protect is the *credibility of the absorber's event*: `ls-remote-lease-late-ack` existed to report that the counterparty had redelivered a packet, and ungated, any sender could forge that report against any Lease. A forgeable signal is worse than no signal, because monitoring eventually keys on it.
>
> PR #765 resolves that tension the other way: the lease-state absorbers emit nothing and therefore authenticate nothing. They reuse `state::ignore_msg` — the same treatment `Closed` and `Liquidated` already gave every other stale message — which returns `Ok` with `MessageResponse::default()`, no state write and no event. The `ls-remote-lease-late-ack` event is gone from every lease-state absorber, `OpenFailed` included. The cost is accepted and named: a redelivery reaching a lease state is no longer observable.
>
> **Unchanged:** callbacks routed through the dex composite still authenticate. `dex::State<H>::on_remote_lease_callback` (`lease/src/contract/state/dex.rs:63`) checks `dex::Handler::authz_remote_callback` before it inspects or dispatches the callback, and every in-flight handler still supplies its permission (`buy_asset`, `buy_lpn`, `sell_asset`, `paid/transfer_in`, `transfer_in_finish`, `OpenLease`). The withdrawal is scoped to absorbers that discard the callback without reporting it.
...
**Progress:** v10 deployment status is settled — §8 Q5 answered 2026-07-31, v10 unreleased with no live leases, which pre-agrees the storage decision as in-place/no-bump (Phase F). The outbound-migration direction remains open, and it is the gating item.
...
> **Amended 2026-08-04 (#763).** Re-scoped under the freeze posture (deterministic errors are bugs; freeze, unchanged, until a fixed deploy — see the §2 amendment). This phase is **no longer a prerequisite for Phase C on the controller legs**: every gap below is reachability-identical before and after the rewire (the wrapper overrode none of `on_error`/`on_response` for these cases either), so the rewire relocates no failure mode. Item-by-item:
>
> - **`SwapExactIn` decode failures** — withdrawn. Q6 is answered as (b): a deterministic decode failure of a SUCCESS ack `Err`s, the ack redelivers, the lease freezes until a fixed deploy — the retained packet commitment is the re-drive, so no recovery state is needed.
> - **`SwapExactIn::heal` / `TransferOut::heal`** — withdrawn as unsafe for `SwapExactIn`: "re-enter/re-attempt" would re-send a swap the vault already executed, and post-#763 there is no buffered response for it to re-drive.
> - **`TransferInInit::on_error`** — DONE (follow-up to #763): implemented as retry, the same posture as `TransferOut::on_error`. A transfer-back rejection leaves the proceeds in the vault, so re-requesting the identical send is safe regardless of cause; every swap error except the liquidation slippage anomaly retries likewise, so the retry posture is uniform across the legs.
> - **`TransferInInit::heal`** — REMOVED (follow-up to #763): it aliased `on_response`, skipping ahead to the balance poll instead of re-driving the pending transfer-back, which stranded the eventual ack on `TransferInFinish`'s default `Err` path. With the wrapper gone there is no lost-response case left for it to recover; `heal` on this leg now correctly reports there is nothing to heal.
> - **`TransferInFinish` late-ack absorber** — stays open but is C3: unreachable under honest IBC (ack redelivery happens only when state did not advance), LOW/informational, untouched by #763.
...
- **`TransferInFinish` missing late/duplicate absorber** (the `HandlerT` impl at `transfer_in_finish.rs:263` defines only `authz_remote_callback`/`heal`/`on_time_alarm` — no `on_response`/`on_error`/`on_timeout` override): a late/duplicate `RemoteLeaseCallback` on the UNORDERED channel hits the default `on_response`/`on_error`/`on_timeout` → `Err` → infinite retry. Add an **idempotent, no-mutation, `Ok`-returning** absorber. Authentication on this path is already wired and stays: `dex::State<H>` checks `authz_remote_callback` before it inspects the callback, and `TransferInFinish` supplies the permission at `:275`. Per the §3 amendment this is **no longer the `OpenFailed` analog** — the lease-state absorbers have since dropped both the check and the event; what carries over is the `Ok`-with-no-mutation return, not the auth wiring.
...
**Progress (2026-08-04, #763):** the transport flip is done for the two controller legs — `SwapExactInRespDelivery` and `TransferInInitRespDelivery` removed, their `on_response` arms rewired to direct `Handler::on_response`, without Phase B (see the §2 amendment). Outstanding here: the `TransferOut` arm (blocked on Phase A) and the `OperationOk` JSON round-trip elimination (`Handler::on_response` still takes `Binary`).
...
Persisted composite `State` is **externally-tagged JSON** (`#[derive(Serialize, Deserialize)]`, default serde), so removing unused `*RespDelivery` variant **names** is safe — no positional index shift.

**§8 Q5 is answered as of 2026-07-31 — v10 is unreleased, no live leases — so the first branch below applies:** in-place change, **no bump**, `migrate` stays refusing (`UnsupportedMigration`) per the wire-format-versioning-in-place precedent (PR #736); update only the migrate doc comment. PR #765 already exercised this branch for the `OpenFailed` variant's shape.

The conditional is kept because this phase executes **at deploy**, and the answer is a statement about today: re-confirm the deployment state before acting on it. **If v10 is by then deployed with live leases:** bump `CONTRACT_STORAGE_VERSION` (`endpoins.rs:29`) to 11 and **drain all leases to a terminal state before upgrade** (flow doc line 174 — there is no `ExecuteMsg` escape hatch; a lease persisted **in** a `*RespDelivery` state would misdeserialize).
...
7. **A sibling funding ack lands while `TransferOutRespDelivery` persists.** *Trigger:* open funds two coins (`acks_left = 2`); coin 1's box-B delivery fails (realistically an oracle outage while quoting the swap floor in `SwapExactIn::enter`), so the wrapper survives its transaction; coin 2's `SudoMsg` then dispatches on the wrapper, whose `on_response`/`on_error`/`on_timeout` are the default `Err` — the sudo fails and Neutron parks the packet in the `contractmanager` failure queue. *Mitigation:* none in code, deliberately — processing the second ack inline would re-run the failing work under the gas cap, and buffering it grows machinery Phase A deletes. The path is self-healing in order: box D retries coin 1 until the transient clears, the state unwraps, and the parked packet is resubmitted via the standard `ResubmitFailure`. Known residual until Phase A removes the leg from sudo entirely.

> **Amended 2026-08-04 (#763).** Items 1 and 2 are re-adjudicated by the §2 amendment: the "infinite relayer retry" they mitigate is not a regression against the pre-removal mechanism (an infinite time-alarm retry with no absorber), and the freeze posture accepts it as the intended behavior for deterministic failures. Item 2's re-drivable recovery state is withdrawn with Q6's answer.
...
5. ~~**Is v10 deployed to any testnet/devnet with live leases** that could sit in an in-flight dex sub-state at upgrade?~~ **ANSWERED 2026-07-31 (via PR #765): no — v10 is unreleased, with no live leases anywhere.** Determines in-place (no bump, migrate stays refusing) vs bump-to-11 + drain-before-upgrade; the answer selects **in-place, no bump**, as recorded in Phase F. Mainnet population is zero per the flow doc, and this extends that to testnet/devnet.

   First applied in PR #765, which removed the `leases: LeasesRef` field from the persisted `OpenFailed` variant in place — `CONTRACT_STORAGE_VERSION` left at 10, `migrate` still refusing. Note this answer is about **today**; it stops holding the moment v10 reaches a chain with live leases, and Phase F must be re-read against the deployment state current at that time rather than against this line.
6. ~~**Recovery target for an undeliverable SUCCESS swap response** (repay/close, `decode_resp.rs:151/172/116`): (a) a new re-drivable "proceeds-unrecoverable" recovery state fixed via `Heal`/migration, (b) accept the `Err` loop until migration, or (c) bounded local retry. The one case with no clean terminal — a product decision.~~ **ANSWERED 2026-08-04 (#763): (b)**, per the freeze posture — a deterministic decode failure is a bug; the ack `Err`s and redelivers against unchanged state until a fixed deploy, whose next redelivery recovers the proceeds with no dedicated state. The pre-#763 mechanism offered nothing stronger: the same failure spun on a `+1ns` time alarm with no terminal either, after the ack had already committed.
