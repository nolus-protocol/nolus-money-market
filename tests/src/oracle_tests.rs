...
fn overwrite_alarm_and_dispatch() {
    ...
    platform::tests::assert_event(
        &res.events,
        &Event::new("wasm-pricealarm-delivery").add_attribute("delivered", "success"),
    );
}
        ...
        platform::message::Response::messages_only({
            ...
            batch.schedule_execute_wasm_no_reply::<_, BaseC>(
                ...
                &ExecuteMsg::AddPriceAlarm {
                    alarm: Alarm::new(
                        price::total_of(common::coin::<BaseC>(base)).is(common::lpn_coin(quote)),
                        None,
                    ),
                },
                None,
...
fn price_alarm_rescheduling() {
    ...
    assert!(
        response
            .events
            .iter()
            .filter(|event| event.ty == "wasm-pricealarm-delivery")
            .all(|event| {
                event
                    .attributes
