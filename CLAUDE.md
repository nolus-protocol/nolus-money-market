...
3. **DEX and remote-swap abstraction**: The `dex` package orchestrates asynchronous swap and transfer workflows (`dex::Account`), building swap requests as `remote_lease::swap::SwapParams`. The `remote_lease` contract is the IBC controller paired with the Solana-side Remote Lease App (ADR 0001/0002); its typed operations and wire types live in the `remote_lease` and `remote_lease_wire` packages. Lease **open** derives its `dex::Account` from the controller's `OpenLease` ack (`RemoteLeaseId` → `platform::ica::HostAccount`) rather than registering a Cosmos ICA; the swap transport and the repay/close collateral **transfer-in** now route over the remote-lease controller (`WasmMsg::Execute` of `remote_lease::msg::ExecuteMsg::Swap` and `ExecuteMsg::TransferOut` respectively, no ICA — transfer-in brings the DEX proceeds back to Nolus and is confirmed by bank-balance polling in `TransferInFinish`), while the outbound transfer-out (Nolus→DEX: funding the account at open and forwarding the repay payment, via `bank_ibc::local::Sender`) still submits over ICA (`submit_tx`).
