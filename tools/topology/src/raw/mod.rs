...
struct RawNetwork<'r> {
    #[serde(borrow)]
    currencies: BTreeMap<&'r str, RawCurrencyWithIcon<'r>>,
    #[serde(default)]
...
pub(crate) struct RawTopology<'r> {
    ...
    networks: BTreeMap<&'r str, RawNetwork<'r>>,
...
impl<'r> RawTopology<'r> {
    ...
    fn all_foreign_currencies<'t, Container>(
        networks: &'t BTreeMap<&str, RawNetwork<'r>>,
        ...
        Container: container::Container,
    {
        channels
            .and_then(|channels| {
                if channels.contains_key(currency.network) {
                    Some(())
                } else {
                    cold();
    ...
    fn validate_non_host_foreign_currency<Container>(
        networks: &BTreeMap<&str, RawNetwork<'r>>,
        currency: &RawForeignCurrency<'r>,
        deduplication_ctx: &mut Container::DeduplicationContext,
    ) -> Result<(), Error<Container>>
    where
    ...
    fn transform_networks<Container>(
        raw: BTreeMap<&'r str, RawNetwork<'r>>,
        deduplication_ctx: &mut Container::DeduplicationContext,
