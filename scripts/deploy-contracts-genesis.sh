...
add_wasm_messages() {
  ...
  _add_wasm_message "$genesis_home_dir" "$wasm_code_path" "treasury" "$((++id))" "$admin_addr" "$treasury_init_tokens"  "--instantiate-only-address $admin_addr" "$treasury_init_msg"
  ...
  local -r lpp_init_msg='{"lpn_ticker":"'$lpp_native'","lease_code_admin":"'$LEASER_ADDRESS'","borrow_rate":{"base_interest_rate":100,"utilization_optimal":750,"addon_optimal_interest_rate":20}}'
  ...
  _add_wasm_message "$genesis_home_dir" "$wasm_code_path" "profit" "$((++id))" "$admin_addr" "" "--instantiate-only-address $admin_addr" "$profit_init_msg"
  _export_to_file "profit" "$PROFIT_ADDRESS" "$contracts_info_file"

  local -r leaser_init_msg='{"lease_code_id":"'$LEASE_CODE_ID'","lease_interest_rate_margin":30,"liability":{"initial":650,"healthy":700,"first_liq_warn":720,"second_liq_warn":750,"third_liq_warn":780,"max":800,"recalc_time":7200000000000},"lpp_ust_addr":"'$LPP_ADDRESS'","time_alarms":"'$TIMEALARMS_ADDRESS'","market_price_oracle":"'$ORACLE_ADDRESS'","profit":"'$PROFIT_ADDRESS'","lease_interest_payment":{"due_period":5184000000000000,"grace_period":864000000000000}}'
  ...
  local -r admin_contract_init_msg='{"general_contracts":{"profit":"'$PROFIT_ADDRESS'","timealarms":"'$TIMEALARMS_ADDRESS'","treasury":"'$TREASURY_ADDRESS'"},"specialized_contracts":{"'$lpp_native'":{"dispatcher":"'$REWARDS_DISPATCHER_ADDRESS'","leaser":"'$LEASER_ADDRESS'","lpp":"'$LPP_ADDRESS'","oracle":"'$ORACLE_ADDRESS'"}}}'
  _add_wasm_message "$genesis_home_dir" "$wasm_code_path" "admin_contract" "$((++id))" "$admin_addr" "" "--instantiate-only-address $admin_addr" "$admin_contract_init_msg"
...
_add_wasm_message() {
  ...
  if [[ $# -eq 8 ]]; then
    ...
    run_cmd "$genesis_home_dir" add-wasm-genesis-message instantiate-contract "$code_id" "$init_msg" --label "$contract_name" \
      --run-as "$admin_addr" --admin "$(admin_contract_instance_addr)" $amount_flag
  fi
}
