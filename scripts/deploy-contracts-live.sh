...
deploy_contracts() {
  ...
  local are_valid
  if ! are_valid=$(_keys_check "$nolus_home_dir"  "$nolus_net" "$dex_admin_wallet_key" "$store_code_privileged_wallet_key");  then
