...
mod test {
    ...
    fn ema_price() {
        ...
        let p1 = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5160));
        let p20 = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5500)); // overwritten
        let p21 = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5000 + 10));
        let p22 = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5000 - 10));
        let p3 = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5000));

        let expect = price::total_of(Coin::<Weth>::new(19)).is(Coin::<Usdc>::new(5010));

        let feeder1 = Addr::unchecked("feeder1");
        let feeder2 = Addr::unchecked("feeder2");

        let mut feed = PriceFeed::new();
        feed = feed.add_observation(feeder1.clone(), s1, p1, config.period());
        feed = feed.add_observation(feeder1.clone(), s21, p20, config.period());
        feed = feed.add_observation(feeder1.clone(), s22, p21, config.period());
        feed = feed.add_observation(feeder2, s22, p22, config.period());
        feed = feed.add_observation(feeder1, s3, p3, config.period());

        assert_eq!(Ok(expect), feed.get_price(&config));
    }
}
