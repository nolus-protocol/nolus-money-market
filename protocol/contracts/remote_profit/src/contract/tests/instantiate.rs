...
use super::{
    CONNECTION_ID, CREATOR, DEX_LABEL, PROFIT_CODE_ID, PROFIT_CONTRACT, TRANSFER_CHANNEL, deps,
    instantiate_msg, query_channel, query_config, sender,
};
...
fn proper_initialization() {
    ...
    assert_eq!(
        external::Code::from(CodeId::from(Code::unchecked(PROFIT_CODE_ID))),
        config.profit_code_id,
    );
    assert_eq!(
        sdk_testing::user(PROFIT_CONTRACT).into_string(),
        config.profit_contract,
    );
...
#[test]
fn instantiate_rejects_malformed_profit_contract() {
    let mut deps = deps();
    let msg = InstantiateMsg {
        profit_contract: "NOT_BECH32!".into(),
        ..instantiate_msg()
    };
    let err = instantiate(deps.as_mut(), testing::mock_env(), sender(CREATOR), msg).unwrap_err();
    assert!(matches!(err, Error::Std(_)), "got {err:?}");
...
#[test]
fn instantiate_rejects_unknown_profit_code() {
