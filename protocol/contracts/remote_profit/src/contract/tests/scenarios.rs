use remote_profit::{
    ...
    response::{OpenProfitResponse, RemoteProfitId, WireOperationResponse},
};
use sdk::{
...
use super::{
    ADMIN, CONNECTION_ID, COUNTERPARTY_CHANNEL_ID, COUNTERPARTY_PORT_ID, LOCAL_CHANNEL_ID,
    PACKET_TIMEOUT, PROFIT, PROFIT_CONTRACT, VERSION, deps, deps_with_profit, instantiate_default,
    sample_open_profit_params, sender,
};
...
fn scenario_open_channel_through_ack_dispatches_callback() {
    ...
    let packet_data = drive_open_profit(deps.as_mut());
...
fn drive_open_profit(deps: DepsMut<'_>) -> Binary {
    ...
    .expect("OpenProfit from authorised profit must succeed");
...
fn assert_callback_to(
    ...
    match &messages[0].msg {
        cosmwasm_std::CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            let expected = cosmwasm_std::to_json_binary(&ProfitExecuteMsg::RemoteProfitCallback(
                expected_callback,
            ))
