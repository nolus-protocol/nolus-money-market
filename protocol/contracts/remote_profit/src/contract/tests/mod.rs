...
const WRONG_CODE_ID: u64 = PROFIT_CODE_ID + 1;
...
// between `execute_outbound` tests (which need the profit address authorised)
// and the first scenario in `scenarios` (which drives an outbound packet
// before consuming the ack).
fn deps_with_profit() -> OwnedDeps<MockStorage, MockApi, MockQuerier> {
    let profit = sdk_testing::user(PROFIT);
    ...
    deps.querier.update_wasm(move |query| match query {
        WasmQuery::ContractInfo { contract_addr } => {
            ...
            if addr == profit {
                contract_info_response(PROFIT_CODE_ID)
            } else if addr == wrong {
