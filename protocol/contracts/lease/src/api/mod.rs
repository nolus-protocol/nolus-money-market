...
pub enum ExecuteMsg {
    ...
    /// - in the final transfer-in step of a repay or close (the state query reports `TransferInFinish`), when the lease has stalled — the repay or close completes once the swap proceeds arrive
