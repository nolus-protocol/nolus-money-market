...
fn ltv_to_price(
    ...
    due: LpnCoin,
) -> impl FnMut(Percent) -> Price<TestCurrency, TestLpn> {
...
fn spec_with_first(warn: Percent, min_asset: Amount, min_transaction: Amount) -> Spec {
    spec_with_max(warn + STEP + STEP + STEP, min_asset, min_transaction)
}

fn spec_with_second(warn: Percent, min_asset: Amount, min_transaction: Amount) -> Spec {
    spec_with_max(warn + STEP + STEP, min_asset, min_transaction)
}

fn spec_with_third(warn: Percent, min_asset: Amount, min_transaction: Amount) -> Spec {
    spec_with_max(warn + STEP, min_asset, min_transaction)
}

// init = 1%, healthy = 1%, first = max - 3, second = max - 2, third = max - 1
fn spec_with_max(max: Percent, min_asset: Amount, min_transaction: Amount) -> Spec {
    let initial = STEP;
