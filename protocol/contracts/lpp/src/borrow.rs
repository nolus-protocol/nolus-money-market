...
impl InterestRate {
    ...
    pub fn calculate<Lpn>(&self, total_liability: Coin<Lpn>, balance: Coin<Lpn>) -> Percent100 {
        ...
        .and_then(|utilization_factor_max| {
            let config = Rational::new(
                self.addon_optimal_interest_rate.units(),
                self.utilization_optimal.units(),
            );
            let may_utilization_factor = if balance.is_zero() {
                Some(utilization_factor_max)
            } else {
                Percent::from_ratio(total_liability, balance)
                    .map(|res| res.min(utilization_factor_max))
            };

            may_utilization_factor
                .and_then(|utilization_factor| config.checked_mul(utilization_factor))
        ...
        .expect("TODO propagate the stack potential overflow")
    }
