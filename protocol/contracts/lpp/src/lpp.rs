...
mod test {
    ...
    #[test]
    fn open_and_repay_with_overflow() {
        let lpp_balance: Amount = 10_000_000;
        let mut deps = testing::mock_dependencies_with_balance(&[coin_cw(lpp_balance)]);
        let mut env = testing::mock_env();
        let admin = Addr::unchecked("admin");
        let lease_addr_1 = Addr::unchecked("loan1");
        let lease_addr_2 = Addr::unchecked("loan2");
        env.block.time = Timestamp::from_nanos(0);
        let lease_code_id = Code::unchecked(123);
        ...
        let borrow_loan1 = Coin::<TheCurrency>::new(5_458_329);
        ...
        Loan::open(&mut deps.storage, lease_addr_1.clone(), &loan1).unwrap();
        ...
        assert_eq!(lpp.total.total_principal_due(), borrow_loan1);
        assert_eq!(
            lpp.total.total_interest_due_by_now(&env.block.time),
            Coin::ZERO
        );
        ...
        let borrow_loan2 = Coin::<TheCurrency>::new(3_543_118);
        ...
        Loan::open(&mut deps.storage, lease_addr_2.clone(), &loan2).unwrap();
        let total_interest_due = lpp.total.total_interest_due_by_now(&env.block.time);
        ...
        assert_eq!(lpp.total.total_principal_due(), borrow_loan1 + borrow_loan2);
        assert_eq!(
            lpp.total.total_interest_due_by_now(&env.block.time),
            total_interest_due
        );
        ...
        let total_due_loan1 = loan1.interest_due(&env.block.time) + loan1.principal_due;
        loan1 = Loan::load(&deps.storage, lease_addr_1.clone()).unwrap();
        let payment1 = loan1.repay(&env.block.time, total_due_loan1);
        Loan::save(&mut deps.storage, lease_addr_1.clone(), loan1.clone()).unwrap();

        assert!(loan1.principal_due.is_zero());
        assert!(loan1.interest_due(&env.block.time).is_zero());
        assert!(payment1.excess.is_zero());
        ...
        let total_due_loan2 = loan2.interest_due(&env.block.time) + loan2.principal_due;
        loan2 = Loan::load(&deps.storage, lease_addr_2.clone()).unwrap();
        let payment2 = loan2.repay(&env.block.time, total_due_loan2);
        Loan::save(&mut deps.storage, lease_addr_2.clone(), loan2.clone()).unwrap();

        assert!(loan2.principal_due.is_zero());
        assert!(loan2.interest_due(&env.block.time).is_zero());
        assert!(payment2.excess.is_zero());
        ...
        assert!(payment2.excess.is_zero());
        assert!(lpp.total_due(&env.block.time).is_zero());
    }
