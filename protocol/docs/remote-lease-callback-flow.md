...
## Storage: v9 → v10 (storage-gated migrate)

The remote-lease feature reshapes the lease's persisted state end to end, and the whole reshape lands as a single v9 → v10 step. The intermediate storage bumps it accumulated during development (v10–v15; PRs #634/#635/#641/#669/#672/#675) were all unreleased — every tagged build ships the lease at v9 — so #673 collapsed them: no deployed lease ever persisted at an intermediate version, and the persisted layout itself is unchanged by the collapse. What v10 encompasses, relative to v9:

- `LeaseDTO.remote_lease_id` becomes a non-optional Solana PDA (binary-incompatible with the v9 layout), and `LeaseDTO` gains the non-optional `remote_lease_controller`, pinned at open, so the post-opening legs emit operations without re-querying the leaser.
- The opening-swap state is reshaped: the `BuyAsset` spec gains the controller address and an opening slippage bound, and the swap leg moves from the ICA `SwapExactIn` to the `RemoteSwap` transport. Since #639 that bound is `MaxSlippages::opening` — the opening swap's own dedicated max-slippage limit, validated independently of (and not ordered against) the liquidation bound — not the liquidation bound it borrowed before.
- `dex::Account.host` becomes optional (`None` on the remote-lease path, which opens no ICA), and the opening and repay funding moves off the ICA transfer-out onto the `Funding` leg (`dex::StateFundRemote`) over the paired transfer channel — both the `Account` shape and the persisted opening/repay state layouts change.
- The `State` enum gains the `OpeningUnwind` in-flight variant (#658, the zero-acked clean-unwind drain) and the `ClosingRemoteLease` variant; the three home-bound drains (the unwind drain, the opened-lease proceeds drain, and the paid-close `TransferOut`) carry a persisted per-currency `baseline` (#670).

The `Liquidated` terminal additionally carries the pinned `remote_lease_controller` (copied from `LeaseDTO`), kept so it can authorise late-after-terminal callbacks like `Closed` — a required field on this same forward-only layout, binary-incompatible with any earlier `Liquidated` payload exactly like the `remote_lease_id` reshape; no such payload can ever be loaded under it, since no lease has ever been persisted under the unreleased remote-lease layouts.

The step is forward-only with no data transform, but **not rollback-safe once a lease persists mid-drain or in `OpeningUnwind`** — an older binary cannot load the new layout. The `migrate` entry point is **gated on the storage version** rather than refusing unconditionally: a record from the pre-remote-lease v9 layout is refused with `ContractError::UnsupportedMigration` (no v9 instance can ever load the remote-lease layout), while a same-storage in-family upgrade — one that keeps the storage version — runs the standard `update_software`, exactly as every sibling contract does. Refusing the incompatible v9 layout is safe and correct because:

- Remote-lease deploys only as a fresh protocol instance (the leaser's `remote_lease_controller` binding is immutable, so an existing protocol cannot be retrofitted), so no existing v9 lease is ever pointed at this code; refusing is strictly safer than risking a silent deserialise failure on the first post-upgrade load.
- A v9 lease has no meaningful `remote_lease_id` to synthesise — its `dex_account` is an ICA host on the DEX chain, not a Solana PDA — and a pre-v10 opening state has no transport to resume on, so a "real" migration would only invent permanent sentinels.

**Operational procedure for non-mainnet (devnet/testnet/local):** drain every pre-v10 lease to a terminal state *before* upgrading the lease code. There is no `ExecuteMsg` escape hatch for a stranded pre-v10 lease, so the drain is a prerequisite, not a recovery step.
