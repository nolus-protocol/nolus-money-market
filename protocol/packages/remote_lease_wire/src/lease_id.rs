...
/// Maximum byte length of a [`RemoteLeaseId`] payload.
///
...
pub const REMOTE_LEASE_ID_MAX_BYTES: usize = 64;

const BASE58_ALPHABET: &[u8] = b"123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

/// Typed wrapper around the Solana lease PDA address travelling on the wire.
///
...
/// non-empty, at most [`REMOTE_LEASE_ID_MAX_BYTES`] bytes, base58 character
...
impl fmt::Display for RemoteLeaseId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(f)
    }
}

impl AsRef<str> for RemoteLeaseId {
    fn as_ref(&self) -> &str {
        self.0.as_str()
    }
}
...
fn validate(value: &str) -> Result<(), Error> {
    ...
    value
        ...
        .map_or(Ok(()), |byte| {
            Err(Error::RemoteLeaseIdInvalidCharacter { byte: *byte })
        })
}

impl Serialize for RemoteLeaseId {
...
impl<'de> Deserialize<'de> for RemoteLeaseId {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_str(RemoteLeaseIdVisitor)
    }
}

struct RemoteLeaseIdVisitor;

impl de::Visitor<'_> for RemoteLeaseIdVisitor {
    type Value = RemoteLeaseId;

    fn expecting(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "a non-empty base58 string of at most {REMOTE_LEASE_ID_MAX_BYTES} bytes"
        )
    }
    ...
    where
        E: de::Error,
    {
        RemoteLeaseId::new(value).map_err(|err| E::custom(err.to_string()))
    }
}
