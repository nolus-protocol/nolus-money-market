...
pub use crate::{
    ...
    release::{
        query, PlatformPackageRelease, ProtocolPackageRelease, ProtocolPackageReleaseId,
        ProtocolReleaseId, SoftwareReleaseId, UpdatablePackage,
    },
    ...
    previous
        ...
        .and_then(|()| {
            migrate_storage(storage).map(|storage_migration_output| FullUpdateOutput {
                to: SoftwareReleaseId::VOID, //TODO remove the release return value!!!
