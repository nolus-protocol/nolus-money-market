...
use crate::{
    protocol::Protocol,
    release::{ProtocolReleaseId, SoftwareReleaseId},
    software::Package,
};
...
pub enum Error {
    ...
    #[error("[Versioning] The package release does not match the currently migration target! The current package's release is \"{0}\", the release being migrated to is \"{1}\".")]
    SoftwareReleaseMismatch(SoftwareReleaseId, SoftwareReleaseId),

    #[error("[Versioning] The protocol release does not match the currently migration target! The current protocol's release is \"{0}\", the release being migrated to is \"{1}\".")]
    ProtocolReleaseMismatch(ProtocolReleaseId, ProtocolReleaseId),
...
impl Error {
    ...
    pub(crate) fn software_release_mismatch(
        current: SoftwareReleaseId,
        expected: SoftwareReleaseId,
    ) -> Self {
        Self::SoftwareReleaseMismatch(current, expected)
    }

    pub(crate) fn protocol_release_mismatch(
        current: ProtocolReleaseId,
        expected: ProtocolReleaseId,
    ) -> Self {
